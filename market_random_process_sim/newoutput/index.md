...
- `figures/exp4_final_price_distribution.png`: Histograms of final prices for baseline and treatment groups.
- `figures/exp4_mean_paths_baseline_vs_inflow.png`: Cross-path mean price paths with 95% confidence bands.
- `figures/exp4_paths_baseline_vs_inflow.png`: Baseline and treatment paths with bold cross-path means, illustrating the upward channel shift.
- `figures/exp4_random_walk_diagnostics_bars.png`: Lag-1 autocorrelation, variance ratio, and positive-return share for baseline vs treatment.
...
- `reports/exp4_findings.md`: Auto-generated Experiment 4 interpretation block.
...
- `tables/exp4_main_comparison.csv`: Main two-group comparison (baseline vs capital-inflow treatment) under the loose cash regime, with Welch t-tests on mean log return.
- `tables/exp4_path_metrics.csv`: Per-path diagnostics for every Experiment 4 simulation (regime, group, run id, return metrics, constraint metrics).
- `tables/exp4_robustness_by_regime.csv`: Robustness table covering tight and loose cash regimes for the same baseline-vs-treatment contrast.
